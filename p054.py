...
def p054(poker_txt):
    hands  = zip([sorted([h for i, h in enumerate(x.split(' ')) if i <  5]) for x in poker_txt.split('\n') if len(x) > 0],
                 [sorted([h for i, h in enumerate(x.split(' ')) if i >= 5]) for x in poker_txt.split('\n') if len(x) > 0])

    values = {'2':1, '3':2, '4':3, '5':4, '6':5, '7':6, '8':7, '9':8, 'T':9, 'J':10, 'Q':11, 'K':12, 'A':13}
    suits  = {'S':1, 'H':2, 'D':3, 'C':4}

    hvalue = lambda h: [values[k[0]] for k in h]
    hsuits = lambda h: [suits[k[1]] for k in h]

    rules  = { 1:('Royal Flush',    lambda h, v, s: sorted(v) == [9, 10, 11, 12, 13] and len(set(s)) == 1,
                                    lambda h, v, s: max(v),
                                    lambda h, v, s: max(s)),
               2:('Straight Flush', lambda h, v, s: sorted(v) == range(min(v), max(v) + 1)
                                              and len(set(s)) == 1,
                                    lambda h, v, s: max(v),
                                    lambda h, v, s: max(s)),
               3:('Four of a Kind', lambda h, v, s: len(set(x for x in v if v.count(x) == 4)) == 1,
                                    lambda h, v, s: max(x for x in v if v.count(x) == 4),
                                    lambda h, v, s: max(s)),
               4:('Full House',     lambda h, v, s: len(set(i for i, x in enumerate(v) if v.count(x) == 2)) == 1
                                                    and len(set(x for i, x in enumerate(s) if i not in \
                                                     set(i for i, x in enumerate(v) if v.count(x) == 2))) == 1,
                                    lambda h, v, s: max(x for x in v if v.count(x) == 2),
                                    lambda h, v, s: max(s)),
               5:('Flush',          lambda h, v, s: len(set(s)) == 1,
                                    lambda h, v, s: max(v),
                                    lambda h, v, s: max(s)),
               6:('Straight',       lambda h, v, s: sorted(v) == range(min(v), max(v) + 1),
                                    lambda h, v, s: max(v),
                                    lambda h, v, s: max(s)),
               7:('Three of a Kind',lambda h, v, s: len(set(x for x in v if v.count(x) == 3)) == 1,
                                    lambda h, v, s: max(x for x in v if v.count(x) == 3),
                                    lambda h, v, s: max(s)),
               8:('Two Pairs',      lambda h, v, s: len(set(x for x in v if v.count(x) == 2)) == 2,
                                    lambda h, v, s: max(set(x for x in v if v.count(x) == 2)),
                                    lambda h, v, s: max(s)),
               9:('One Pair',       lambda h, v, s: len(set(x for x in v if v.count(x) == 2)) == 1,
                                    lambda h, v, s: max(x for x in v if v.count(x) == 2),
                                    lambda h, v, s: max(s)),
              10:('High Card',      lambda h, v, s: True,
                                    lambda h, v, s: max(v),
                                    lambda h, v, s: max(s))
             }
    ...
    for p1, p2 in hands:
        ...
        for r in sorted(rules):
            if rules[r][1](p1, hvalue(p1), hsuits(p1)):
                p1_score = (10 - r, rules[r][0])
                p1_maxv  = rules[r][2](p1, hvalue(p1), hsuits(p1))
                p1_maxs  = rules[r][3](p1, hvalue(p1), hsuits(p1))
                break

        for r in sorted(rules):
            if rules[r][1](p2, hvalue(p2), hsuits(p2)):
                p2_score = (10 - r, rules[r][0])
                p2_maxv  = rules[r][2](p2, hvalue(p2), hsuits(p2))
                p2_maxs  = rules[r][3](p2, hvalue(p2), hsuits(p2))
                break
        ...
        if p1_score[0] == p2_score[0]:
            if p1_maxv > p2_maxv:
                rounds += ['P1']
            elif p1_maxv < p2_maxv:
                rounds += ['P2']
            else:
                if p1_maxs > p2_maxs:
                    rounds += ['P1']
                elif p1_maxs < p2_maxs:
                    rounds += ['P2']
                else:
                    rounds += ['D']

    return sum(1 for x in rounds if x == 'P1')

print(p054(poker_txt))
