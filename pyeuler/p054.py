...
def p054(poker_txt):
    ...
    hands  = zip([sorted([h for i, h in enumerate(x.split(' ')) if i <  5])
                              for x in poker_txt.split('\n') if len(x) > 0],
                 [sorted([h for i, h in enumerate(x.split(' ')) if i >= 5])
                              for x in poker_txt.split('\n') if len(x) > 0])
    ...
    rules  = { 1:('Royal Flush',    lambda h,v,s: sorted(v) == [9, 10, 11, 12, 13] \
                                                    and len(set(s)) == 1,
                                    lambda h,v,s: max(v), lambda h,v,s: max(s)),
               2:('Straight Flush', lambda h,v,s: sorted(v) == range(min(v), max(v) + 1)
                                                    and len(set(s)) == 1,
                                    lambda h,v,s: max(v), lambda h,v,s: max(s)),
               3:('Four of a Kind', lambda h,v,s: len(set(x for x in v \
                                                                if v.count(x) == 4)) == 1,
                                    lambda h,v,s: max(x for x in v if v.count(x) == 4),
                                    lambda h,v,s: max(s)),
               4:('Full House',     lambda h,v,s: len(set(i for i, x in enumerate(v) \
                                                                if v.count(x) == 2)) == 1
                                                    and len(set(x for i, x in enumerate(s) \
                                                    if i not in set(i for i, x in enumerate(v) \
                                                                if v.count(x) == 2))) == 1,
    ...
    evaluate = lambda p, r: rules[r][1](p, hvalue(p), hsuits(p))
    score    = lambda p, r: ((10 - r, rules[r][0]),
                             rules[r][2](p1, hvalue(p), hsuits(p)),
                             rules[r][3](p1, hvalue(p), hsuits(p)))
    rounds   = []

    for p1, p2 in hands:
        p1_score, p1_maxv, p1_maxs  = '', 0, 0  #score of hand, max value card, max suit card
        p1_score, p1_maxv, p1_maxs  = '', 0, 0
        for r in sorted(rules):
            if evaluate(p1, r):
                p1_score, p1_maxv, p1_maxs  = score(p1, r)
                break

        for r in sorted(rules):
            if evaluate(p2, r):
                p2_score, p2_maxv, p2_maxs  = score(p2, r)
                break
